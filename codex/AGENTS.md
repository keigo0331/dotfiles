...
- 作業は論理的に分解し、必要最小限のまとまりごとに進める。各作業単位で依頼者にレビューを仰ぎ、問題ないことを確認してからコミットし、次の作業へ進む。

## Git運用
### ブランチ運用
- 作業開始時は最新の main ブランチからブランチを切り、作業を始める。履歴の分岐を最小化するため。
- 調査のみの作業でも必ずブランチを切る。後続の変更と切り分けるため。
- 「ここからブランチを切って」などの指示があった場合は、現在のブランチから新しくブランチを切る。main に戻る必要はない。
- main ブランチへ直接コミットしない。必ずブランチを切り、プルリクエストを経由する。
- main ブランチに戻った際は必ず通常の `git pull` を実行し、`--ff-only` は使用しない。マージコミットを含む最新状態を正しく反映するため。
...
- `git push` や `gh` 実行時にエラーが発生した場合は、依頼者がそのまま実行できるコマンド（例: `git push`、`gh pr create ...`）を回答に明記する。
- PR本文の作成時、改行したいときは改行文字を使わずそのまま改行し、ヒアドキュメントやシェル関数などで作ろうとせずに如実にテキストを作る。また、`gh pr create` の `--body-file` オプションは使用しない。
